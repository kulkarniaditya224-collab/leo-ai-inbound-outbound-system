...
async def insert_appointment(name: str, phone: str, date: str, time: str, service: str, direction: str = "outbound") -> str:
    ...
    await db.table("appointments").insert({
        ...
        "status": "booked", "direction": direction,
...
async def get_all_appointments(date_filter: Optional[str] = None, direction_filter: Optional[str] = None) -> list:
    db = await _adb()
...
async def log_call(
    ...
    duration_seconds: int, direction: str = "outbound",
    recording_url: Optional[str] = None, notes: Optional[str] = None,
    agent_profile_id: Optional[str] = None,
) -> None:
    db = await _adb()
    row: dict = {
        "id": str(uuid.uuid4()), "phone_number": phone_number, "lead_name": lead_name,
        "direction": direction, "outcome": outcome, "reason": reason,
        "duration_seconds": duration_seconds, "timestamp": datetime.now().isoformat(),
    }
    if recording_url:
    ...
    for row in rows:
        ...
        if phone not in contacts:
            contacts[phone] = {
                ...
                "total_calls": 0, "inbound": 0, "outbound": 0, "booked": 0,
                "last_call": row["timestamp"], "last_outcome": row.get("outcome"),
                "last_direction": row.get("direction", "outbound"),
            }
        contacts[phone]["total_calls"] += 1
        d = row.get("direction", "outbound")
        if d == "inbound":
            contacts[phone]["inbound"] += 1
        else:
            contacts[phone]["outbound"] += 1
...
async def get_stats(direction: Optional[str] = None) -> dict:
    db = await _adb()
    ...
    inbound_count  = sum(1 for r in rows if r.get("direction") == "inbound")
    outbound_count = sum(1 for r in rows if r.get("direction") == "outbound")
    ...
    daily_inbound: dict = defaultdict(int)
    daily_outbound: dict = defaultdict(int)
    for r in rows:
    ...
    for i in range(13, -1, -1):
        ...
        timeline.append({"date": d, "total": daily.get(d, 0), "inbound": daily_inbound.get(d, 0), "outbound": daily_outbound.get(d, 0)})
    ...
    return {
        "total_calls": total_calls, "inbound_count": inbound_count, "outbound_count": outbound_count,
...
async def create_agent_profile(
    ...
    mode: str = "both", system_prompt: Optional[str] = None,
    enabled_tools: str = "[]", is_default: bool = False,
) -> str:
    ...
    await db.table("agent_profiles").insert({
        ...
        "mode": mode, "system_prompt": system_prompt, "enabled_tools": enabled_tools,
...
async def create_inbound_route(
    name: str, trunk_id: str, phone_number: str,
    agent_profile_id: Optional[str] = None, greeting: Optional[str] = None,
    system_prompt: Optional[str] = None,
) -> str:
    route_id = str(uuid.uuid4())
    db = await _adb()
    row: dict = {
        "id": route_id, "name": name, "trunk_id": trunk_id,
        "phone_number": phone_number, "status": "active",
        "total_calls": 0, "created_at": datetime.now().isoformat(),
    }
    if agent_profile_id:
        row["agent_profile_id"] = agent_profile_id
    if greeting:
    ...
    if system_prompt:
        row["system_prompt"] = system_prompt
    await db.table("inbound_routes").insert(row).execute()
    return route_id


async def get_all_inbound_routes() -> list:
    db = await _adb()
    result = await db.table("inbound_routes").select("*").order("created_at", desc=True).execute()
    return result.data or []


async def get_inbound_route(route_id: str) -> Optional[dict]:
    db = await _adb()
    result = await db.table("inbound_routes").select("*").eq("id", route_id).maybe_single().execute()
    return result.data if result else None


async def get_inbound_route_by_trunk(trunk_id: str) -> Optional[dict]:
    db = await _adb()
    result = await db.table("inbound_routes").select("*").eq("trunk_id", trunk_id).eq("status", "active").maybe_single().execute()
    return result.data if result else None


async def update_inbound_route(route_id: str, updates: dict) -> bool:
    db = await _adb()
    result = await db.table("inbound_routes").update(updates).eq("id", route_id).execute()
    return len(result.data or []) > 0


async def delete_inbound_route(route_id: str) -> bool:
    db = await _adb()
    result = await db.table("inbound_routes").delete().eq("id", route_id).execute()
    return len(result.data or []) > 0


async def increment_inbound_route_calls(route_id: str) -> None:
    db = await _adb()
    route = await get_inbound_route(route_id)
    if route:
        new_total = (route.get("total_calls") or 0) + 1
        await db.table("inbound_routes").update({"total_calls": new_total}).eq("id", route_id).execute()


# ── FAQ ───────────────────────────────────────────────────────────────────────

async def create_faq(question: str, answer: str, category: str = "general", priority: int = 0) -> str:
    faq_id = str(uuid.uuid4())
    db = await _adb()
    await db.table("faq_entries").insert({
...
async def get_all_faqs(category: Optional[str] = None) -> list:
    db = await _adb()
    query = db.table("faq_entries").select("*").order("priority", desc=True).order("created_at")
    if category:
...
async def update_faq(faq_id: str, updates: dict) -> bool:
    db = await _adb()
    result = await db.table("faq_entries").update(updates).eq("id", faq_id).execute()
    return len(result.data or []) > 0


async def delete_faq(faq_id: str) -> bool:
    db = await _adb()
    result = await db.table("faq_entries").delete().eq("id", faq_id).execute()
    return len(result.data or []) > 0


async def search_faq(query: str) -> list:
    """Simple keyword search across FAQ entries."""
    ...
    for faq in all_faqs:
        ...
        for word in query_lower.split():
            ...
            if word in cat:
                score += 2
        if score > 0:
            results.append({**faq, "_score": score})
