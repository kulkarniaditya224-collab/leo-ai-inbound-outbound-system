...
logger = logging.getLogger("leo-tools")
...
class LeoTools(llm.ToolContext):
    """All function tools available to the LEO AI voice agent — inbound & outbound."""

    def __init__(
        self, ctx: agents.JobContext,
    ...
    async def check_availability(self, date: str, time: str) -> str:
        ...
        Call this BEFORE attempting to book whenever the caller proposes a date/time.
    ...
    async def book_appointment(self, name: str, phone: str, date: str, time: str, service: str) -> str:
        """
        Book an appointment after the caller has verbally confirmed date, time, and service.
        Call ONLY after the caller confirms all details.
        name: full name | phone: with country code | date: YYYY-MM-DD | time: HH:MM | service: type
        """
        try:
            booking_id = await insert_appointment(name, phone, date, time, service, direction=self.direction)
    ...
    async def end_call(self, outcome: str, reason: str = "") -> str:
        ...
        outcome: 'booked' | 'not_interested' | 'wrong_number' | 'voicemail' | 'no_answer' | 'callback_requested' | 'inquiry_resolved' | 'transferred' | 'complaint'
        ...
        try:
            await log_call(
                ...
                duration_seconds=duration, direction=self.direction,
    ...
    async def transfer_to_human(self, reason: str) -> str:
        ...
        Call when the caller requests a human, is angry, or has a complex issue.
    ...
    async def send_sms_confirmation(self, phone: str, message: str) -> str:
        ...
        phone: caller's phone | message: text to send
        """
    ...
    async def lookup_contact(self, phone: str) -> str:
        ...
        phone: the caller's phone number with country code
        ...
        try:
            ...
            if calls:
                ...
                for c in calls[:5]:
                    ...
                    d = c.get("direction", "?")
                    lines.append(f"  • [{d}] {ts} — {c.get('outcome','?')}: {c.get('reason','')}")
            if appointments:
    ...
    async def remember_details(self, insight: str) -> str:
        """
        Store a key insight about this caller for future calls.
    ...
    async def _compress_memories(self) -> None:
        try:
            ...
            prompt = f"Compress these notes about a contact into 3-5 concise bullets. Keep all key facts.\n\n{bullet_list}"
            loop = asyncio.get_event_loop()
    ...
    async def book_calcom(self, name: str, email: str, date: str, start_time: str, notes: str = "") -> str:
        ...
        name: full name | email: caller's email | date: YYYY-MM-DD | start_time: HH:MM | notes: optional
        """
            ...
            async with httpx.AsyncClient(timeout=15) as client:
                resp = await client.post(
                    ...
                    json={"eventTypeId": int(event_type_id), "start": start_iso, "timeZone": timezone,
                          ...
                          "metadata": {"source": "LEO AI"}, "language": "en"},
                )
            data = resp.json()
    ...
    async def lookup_faq(self, question: str) -> str:
        ...
        Use this when the caller asks a general question about the business, services, pricing, hours, etc.
        question: the caller's question or topic
        Returns matching FAQ entries or 'no matching FAQ found'.
        """
        try:
            results = await search_faq(question)
            if not results:
                return "No matching FAQ found. You may need to answer from general knowledge or offer to transfer to a team member."
            lines = ["Here are the relevant answers from our knowledge base:"]
            for i, faq in enumerate(results[:3], 1):
                lines.append(f"\n{i}. Q: {faq['question']}")
                lines.append(f"   A: {faq['answer']}")
            lines.append("\nUse these answers naturally in your own words — don't read them verbatim.")
            return "\n".join(lines)
        except Exception:
            return "Unable to search FAQ right now."
