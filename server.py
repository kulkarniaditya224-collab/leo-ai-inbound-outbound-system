"""LEO AI — FastAPI backend for inbound + outbound voice AI platform."""
...
from db import (
    ...
    create_inbound_route, get_all_inbound_routes, get_inbound_route, update_inbound_route, delete_inbound_route,
    create_faq, get_all_faqs, update_faq, delete_faq,
)
from prompts import DEFAULT_OUTBOUND_PROMPT, DEFAULT_INBOUND_PROMPT
...
app = FastAPI(title="LEO AI Platform", version="2.0.0")
...
async def api_dispatch_call(req: CallRequest):
    ...
    if not effective_prompt:
        effective_prompt = await get_setting("system_prompt_outbound", "") or None

    room_name = f"out-{phone.replace('+', '')}-{random.randint(1000, 9999)}"
    metadata: dict = {
        "phone_number": phone,
    ...
    try:
        ...
        await lk.agent_dispatch.create_dispatch(
            lk_api.CreateAgentDispatchRequest(
                agent_name="leo-ai", room=room_name, metadata=json.dumps(metadata)
            )
        )
        await lk.aclose()
        await session.close()
        await log_error("server", f"Outbound call dispatched to {phone}", f"room={room_name}", "info")
        return {"status": "dispatched", "room": room_name, "phone": phone, "direction": "outbound"}
...
@app.get("/api/stats")
async def api_get_stats(direction: Optional[str] = None):
    return await get_stats(direction=direction)


# ── Appointments ──────────────────────────────────────────────────────────────

@app.get("/api/appointments")
async def api_get_appointments(date: Optional[str] = None, direction: Optional[str] = None):
    return await get_all_appointments(date_filter=date, direction_filter=direction)
...
@app.get("/api/prompt")
async def api_get_prompt(mode: str = "outbound"):
    key = f"system_prompt_{mode}"
    saved = await get_setting(key, "")
    default = DEFAULT_OUTBOUND_PROMPT if mode == "outbound" else DEFAULT_INBOUND_PROMPT
    return {"prompt": saved or default, "is_custom": bool(saved), "mode": mode}


@app.post("/api/prompt")
async def api_save_prompt(req: PromptRequest):
    key = f"system_prompt_{req.mode}"
    await set_setting(key, req.prompt)
    return {"status": "saved", "mode": req.mode}


@app.delete("/api/prompt")
async def api_reset_prompt(mode: str = "outbound"):
    key = f"system_prompt_{mode}"
    await set_setting(key, "")
    default = DEFAULT_OUTBOUND_PROMPT if mode == "outbound" else DEFAULT_INBOUND_PROMPT
    return {"status": "reset", "prompt": default, "mode": mode}
...
@app.post("/api/setup/outbound-trunk")
async def api_setup_outbound_trunk():
    url    = await eff("LIVEKIT_URL")
    key    = await eff("LIVEKIT_API_KEY")
    secret = await eff("LIVEKIT_API_SECRET")
    ...
    try:
        ...
        if not os.getenv("OUTBOUND_TRUNK_ID"):
            os.environ["OUTBOUND_TRUNK_ID"] = trunk_id
        await lk.aclose()
        await session.close()
        return {"status": "created", "trunk_id": trunk_id, "type": "outbound"}
    except Exception as exc:
        raise HTTPException(500, f"Outbound trunk creation failed: {exc}")


# ── SIP trunk setup (Inbound) ────────────────────────────────────────────────

@app.post("/api/setup/inbound-trunk")
async def api_setup_inbound_trunk(req: InboundRouteRequest):
    url    = await eff("LIVEKIT_URL")
    key    = await eff("LIVEKIT_API_KEY")
    secret = await eff("LIVEKIT_API_SECRET")
    sip_domain = await eff("VOBIZ_SIP_DOMAIN")
    ...
    if not all([url, key, secret, sip_domain, username, password]):
        raise HTTPException(400, "Configure LiveKit and Vobiz credentials in Settings first.")
    ...
    try:
        ...
        trunk = await lk.sip.create_sip_inbound_trunk(
            lk_api.CreateSIPInboundTrunkRequest(
                trunk=lk_api.SIPInboundTrunkInfo(
                    name=f"LEO AI Inbound — {req.name}",
                    numbers=[phone],
                    auth_username=username,
                    auth_password=password,
                )
        ...
        # Create dispatch rule so agent auto-joins when call arrives
        await lk.sip.create_sip_dispatch_rule(
            lk_api.CreateSIPDispatchRuleRequest(
                ...
                trunk_ids=[trunk_id],
                name=f"LEO AI Dispatch — {req.name}",
                metadata=json.dumps({
                    "direction": "inbound",
        ...
        # Save route in DB
        route_id = await create_inbound_route(
            name=req.name, trunk_id=trunk_id, phone_number=phone,
            agent_profile_id=req.agent_profile_id, greeting=req.greeting,
            system_prompt=req.system_prompt,
        )

        return {"status": "created", "trunk_id": trunk_id, "route_id": route_id, "type": "inbound"}
    except Exception as exc:
        logger.error("Inbound trunk creation failed: %s", exc)
...
async def api_list_inbound_routes():
    return await get_all_inbound_routes()


@app.get("/api/inbound-routes/{route_id}")
async def api_get_inbound_route(route_id: str):
    route = await get_inbound_route(route_id)
    if not route:
        raise HTTPException(404, "Route not found")
    return route


@app.put("/api/inbound-routes/{route_id}")
async def api_update_inbound_route(route_id: str, req: InboundRouteRequest):
    ok = await update_inbound_route(route_id, {
        "name": req.name, "phone_number": req.phone_number,
        "agent_profile_id": req.agent_profile_id, "greeting": req.greeting,
        "system_prompt": req.system_prompt,
    })
    if not ok:
...
@app.delete("/api/inbound-routes/{route_id}")
async def api_delete_inbound_route(route_id: str):
    ok = await delete_inbound_route(route_id)
    if not ok:
        raise HTTPException(404, "Route not found")
...
@app.get("/api/faq")
async def api_list_faq(category: Optional[str] = None):
    return await get_all_faqs(category=category)


@app.post("/api/faq")
async def api_create_faq(req: FaqRequest):
    faq_id = await create_faq(question=req.question, answer=req.answer, category=req.category, priority=req.priority)
    return {"status": "created", "id": faq_id}


@app.put("/api/faq/{faq_id}")
async def api_update_faq(faq_id: str, req: FaqRequest):
    ok = await update_faq(faq_id, {"question": req.question, "answer": req.answer, "category": req.category, "priority": req.priority})
    if not ok:
...
async def api_create_agent_profile(req: AgentProfileRequest):
    try:
        profile_id = await create_agent_profile(
            name=req.name, voice=req.voice, model=req.model, mode=req.mode,
...
async def _dispatch_one(lk, lk_api, contact: dict, room_name: str,
    ...
    try:
        saved_prompt = prompt or (await get_setting("system_prompt_outbound", "")) or None
        metadata: dict = {
        ...
        await lk.agent_dispatch.create_dispatch(
            lk_api.CreateAgentDispatchRequest(agent_name="leo-ai", room=room_name, metadata=json.dumps(metadata))
        )
        return True
    except Exception as exc:
